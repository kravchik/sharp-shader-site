---
layout: default
title: SharpShader Docs
section: docs
nav_order: 10
nav_title: Docs Index
---

# SharpShader Docs

Public documentation for the current beta surface:

- [Beta scope](./beta-scope.html)
- [Supported environment matrix](./environment-matrix.html)
- [Quick start](../quick-start.html)
- [Technical details](./technical-details.html)
- [Changelog](../changelog.html)
- [Authoring model](./authoring-model.html)
- [C# to HLSL types](./type-mapping.html)
- [Intrinsics and helpers](./intrinsics-and-helpers.html)
- [Resources](./resources.html)
- [Generation workflow](./generation-workflow.html)
- [Examples](./examples.html)
- [Troubleshooting](./troubleshooting.html)

The current docs intentionally stay within the validated beta contract.
If a workflow or feature is not described here, it should not be treated as part of the supported public beta surface yet.
